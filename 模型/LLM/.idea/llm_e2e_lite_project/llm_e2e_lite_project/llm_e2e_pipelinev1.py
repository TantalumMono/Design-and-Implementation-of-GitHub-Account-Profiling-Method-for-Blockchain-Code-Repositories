...
def load_model_and_tokenizer(cfg: Dict):
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load tokenizer from '{model_source}'. "
            f"local_files_only={local_files_only}. "
            "If your network is unstable, download the model to a local folder first "
            "and set model.local_model_dir in the yaml config."
        ) from e
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load model from '{model_source}'. "
            f"local_files_only={local_files_only}. "
            "If your network is unstable, download the model to a local folder first "
            "and set model.local_model_dir in the yaml config."
        ) from e
...
def make_training_args(cfg: Dict, out_dir: str):
    ...
    Compatible with transformers variants that may use:
    - overwrite_output_dir / no overwrite_output_dir
    - eval_strategy / evaluation_strategy
    - bf16 / fp16 availability differences
    """
    ...
    if "lr_scheduler_type" in params:
        kwargs["lr_scheduler_type"] = str(cfg["model"]["lr_scheduler_type"])
...
def make_trainer_kwargs(tokenizer):
    """
    Compatible with transformers variants that use either:
