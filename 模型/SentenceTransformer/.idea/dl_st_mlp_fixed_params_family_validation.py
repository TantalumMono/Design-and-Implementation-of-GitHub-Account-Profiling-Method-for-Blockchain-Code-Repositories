...
OUTPUT_DIR = SCRIPT_DIR / "dl_st_mlp_fixed_params_family_validation_outputs"
...
def main():
    ...
    for outer_fold_id, ((_, outer_test_fams), (_, outer_test_neg_idx)) in enumerate(zip(outer_pos_family_folds, outer_neg_folds), start=1):
        ...
        model, _ = train_one_fold(
            X_tr[train_sub_idx], y_tr[train_sub_idx], w_tr[train_sub_idx],
            X_tr[val_sub_idx], y_tr[val_sub_idx],
            config, device
        )
        ...
        fold_metrics.append({
            ...
            "model": "SentenceTransformer+MLP (fixed params, offline)",
        ...
        tmp["model"] = "SentenceTransformer+MLP (fixed params, offline)"
    ...
    fold_metrics_df.to_csv(OUTPUT_DIR / "dl_st_mlp_fixed_params_fold_metrics.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(summary_rows).to_csv(OUTPUT_DIR / "dl_st_mlp_fixed_params_summary_mean_std.csv", index=False, encoding="utf-8-sig")

    oof_df = pd.concat(oof_records, axis=0, ignore_index=True)
    oof_df.to_csv(OUTPUT_DIR / "dl_st_mlp_fixed_params_oof_predictions.csv", index=False, encoding="utf-8-sig")
    ...
    with open(OUTPUT_DIR / "dl_st_mlp_fixed_params_report.json", "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)

    print("\\nSaved outputs to:", OUTPUT_DIR)
    print("- dl_st_mlp_fixed_params_fold_metrics.csv")
    print("- dl_st_mlp_fixed_params_summary_mean_std.csv")
    print("- dl_st_mlp_fixed_params_oof_predictions.csv")
    print("- dl_st_mlp_fixed_params_report.json")
