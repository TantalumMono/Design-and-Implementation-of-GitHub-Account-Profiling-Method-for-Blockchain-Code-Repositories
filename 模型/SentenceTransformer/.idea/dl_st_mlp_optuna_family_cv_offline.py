...
OUTPUT_DIR = SCRIPT_DIR / "dl_st_mlp_optuna_offline_outputs_true_pos"
...
INNER_N_SPLITS = 3
N_TRIALS = 40
ALLOW_GENERATED_POSITIVES_IN_OUTER_TEST = False
...
def train_one_fold(X_tr, y_tr, w_tr, X_val, y_val, config: TrainConfig, device: torch.device):
    ...
    with torch.no_grad():
        for xb, _, _ in val_loader:
            ...
            probs = torch.sigmoid(model(xb)).cpu().numpy()
            val_probs.append(probs)
    return model, np.concatenate(val_probs)
...
def main():
    ...
    for outer_fold_id, ((_, outer_test_fams), (_, outer_test_neg_idx)) in enumerate(zip(outer_pos_family_folds, outer_neg_folds), start=1):
        ...
        def objective(trial: optuna.Trial) -> float:
            ...
            for ((_, inner_val_fams), (_, inner_val_neg_idx)) in zip(inner_pos_family_folds, inner_neg_folds):
                ...
                X_tr = np.concatenate([readme_emb_all[tr_idx], aux_emb_all[tr_idx], X_tr_num], axis=1).astype(np.float32)
                ...
                w_tr = build_sample_weights(train_df)

                _, val_probs = train_one_fold(X_tr, y_tr, w_tr, X_val, y_val, config, device)
            ...
            thr, thr_info = select_best_threshold_f1_priority(pooled_df["label"].values, pooled_df["y_proba"].values, beta=BETA)
        ...
        print(
            f"Best trial | hidden={best_params['hidden_dim']} layers={best_params['num_layers']} "
            f"dropout={best_params['dropout']:.3f} lr={best_params['lr']:.6f} "
            f"wd={best_params['weight_decay']:.6f} bs={best_params['batch_size']} "
            f"pos_scale={best_params['pos_weight_scale']:.3f} thr={best_threshold:.6f} "
        ...
        best_params_by_fold.append({
            ...
            **best_params,
            ...
            "inner_precision": float(best_trial.user_attrs["precision"]),
            "inner_recall": float(best_trial.user_attrs["recall"]),
            "inner_f1": float(best_trial.user_attrs["f1"]),
        })
        ...
        config = TrainConfig(
            ...
            pos_weight_scale=float(best_params["pos_weight_scale"]),
        )

        rng = np.random.default_rng(RANDOM_STATE + outer_fold_id)
        perm = rng.permutation(len(X_tr))
        split = max(1, int(0.15 * len(X_tr)))
        val_sub_idx = perm[:split]
        train_sub_idx = perm[split:] if split < len(X_tr) else perm[:]

        model, _ = train_one_fold(X_tr[train_sub_idx], y_tr[train_sub_idx], w_tr[train_sub_idx], X_tr[val_sub_idx], y_tr[val_sub_idx], config, device)
        test_proba = predict_probs(model, X_te, config.batch_size, device)
        ...
        fold_metrics.append({
            "outer_fold": outer_fold_id,
            "model": "SentenceTransformer+MLP+Optuna (offline)",
        ...
        tmp["model"] = "SentenceTransformer+MLP+Optuna (offline)"
        ...
        study.trials_dataframe().to_csv(OUTPUT_DIR / f"outer_fold_{outer_fold_id}_optuna_trials.csv", index=False, encoding="utf-8-sig")

    fold_metrics_df = pd.DataFrame(fold_metrics)
    fold_metrics_df.to_csv(OUTPUT_DIR / "dl_st_mlp_offline_fold_metrics.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(summary_rows).to_csv(OUTPUT_DIR / "dl_st_mlp_offline_summary_mean_std.csv", index=False, encoding="utf-8-sig")

    oof_df = pd.concat(oof_records, axis=0, ignore_index=True)
    oof_df.to_csv(OUTPUT_DIR / "dl_st_mlp_offline_oof_predictions.csv", index=False, encoding="utf-8-sig")
    ...
    with open(OUTPUT_DIR / "dl_st_mlp_offline_report.json", "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)

    print("\\nSaved outputs to:", OUTPUT_DIR)
    print("- dl_st_mlp_offline_fold_metrics.csv")
    print("- dl_st_mlp_offline_summary_mean_std.csv")
    print("- dl_st_mlp_offline_oof_predictions.csv")
    print("- dl_st_mlp_offline_report.json")
