...
OUTPUT_DIR = SCRIPT_DIR / "linear_svc_calibrated_family_cv_outputs"
...
def build_sample_weights(df: pd.DataFrame):
    w = np.full(len(df), 1.25, dtype=float)
...
def build_matrices(
    ...
    readme_max_features: int = 400,
    readme_min_df: int = 3,
    readme_ngram_range: tuple = (1, 1),
    aux_max_features: int = 200,
    aux_min_df: int = 2,
    aux_ngram_range: tuple = (1, 2),
):
...
def fit_calibrated_svc(X_tr, y_tr, sample_weight, C_value: float):
    base = LinearSVC(
        C=C_value,
        penalty="l2",
        loss="squared_hinge",
...
def main():
    ...
    for outer_fold_id, ((outer_train_fams, outer_test_fams), (_, outer_test_neg_idx)) in enumerate(
        ...
        for C_value in C_GRID:
            ...
            for inner_id, ((inner_train_fams, inner_val_fams), (_, inner_val_neg_idx)) in enumerate(
                ...
                X_tr, X_val, _ = build_matrices(train_df, val_df, structured_cols)
                y_tr = train_df["label"].values
                sw = build_sample_weights(train_df)

                clf = fit_calibrated_svc(X_tr, y_tr, sw, C_value)
            ...
            if (best_score is None) or (score > best_score) or (np.isclose(score, best_score) and thr_info["recall"] > best_thr_info["recall"]):
                best_score = score
                best_C = C_value
                best_threshold = thr
                best_thr_info = thr_info

        print(f"Best inner C = {best_C} | threshold = {best_threshold:.6f} | "
              f"P={best_thr_info['precision']:.4f} R={best_thr_info['recall']:.4f} F1={best_thr_info['f1']:.4f}")

        test_df = df.loc[outer_test_idx].copy()
        train_df = outer_train_df.copy()

        X_tr_final, X_te_final, artifacts = build_matrices(train_df, test_df, structured_cols)
        y_tr_final = train_df["label"].values
        y_te = test_df["label"].values
        sw_final = build_sample_weights(train_df)

        final_clf = fit_calibrated_svc(X_tr_final, y_tr_final, sw_final, best_C)
        ...
        fold_metrics.append({
            ...
            "val_precision_at_thr": float(best_thr_info["precision"]),
            "val_recall_at_thr": float(best_thr_info["recall"]),
            "val_f1_at_thr": float(best_thr_info["f1"]),
        ...
        tmp["model"] = "LinearSVC+CalibratedClassifierCV"
    ...
    fold_metrics_df.to_csv(OUTPUT_DIR / "linear_svc_calibrated_fold_metrics.csv", index=False, encoding="utf-8-sig")
    ...
    pd.DataFrame(summary_rows).to_csv(
        OUTPUT_DIR / "linear_svc_calibrated_summary_mean_std.csv",
        index=False,
        encoding="utf-8-sig",
    )

    oof_df = pd.concat(oof_records, axis=0, ignore_index=True)
    oof_df.to_csv(OUTPUT_DIR / "linear_svc_calibrated_oof_predictions.csv", index=False, encoding="utf-8-sig")
    ...
    with open(OUTPUT_DIR / "linear_svc_calibrated_report.json", "w", encoding="utf-8") as f:
        json.dump(report, f, ensure_ascii=False, indent=2)

    print("\\nSaved outputs to:", OUTPUT_DIR)
    print("- linear_svc_calibrated_fold_metrics.csv")
    print("- linear_svc_calibrated_summary_mean_std.csv")
    print("- linear_svc_calibrated_oof_predictions.csv")
    print("- linear_svc_calibrated_report.json")
