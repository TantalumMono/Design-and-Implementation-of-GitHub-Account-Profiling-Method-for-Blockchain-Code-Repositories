...
OUTPUT_DIR = SCRIPT_DIR / "final_model_artifacts"
...
FINAL_PARAMS = {
    ...
    # 由固定参数模型的 OOF 概率重新校准得到
    "threshold": 0.039498057,
}
...
def fillna_by_train_statistics(X_tr: pd.DataFrame, strategy: str = "median"):
    if strategy == "median":
...
def main():
    ...
    X_num, fill_values = fillna_by_train_statistics(X_num, strategy=FILLNA_STRATEGY)
    X_num_sp = csr_matrix(X_num.to_numpy(dtype=np.float32))
    ...
    model.fit(X_all, y_all, sample_weight=sample_weight, verbose=False)
    ...
    out_df.to_csv(OUTPUT_DIR / "final_model_trainset_predictions.csv", index=False, encoding="utf-8-sig")
    ...
    joblib.dump(bundle, OUTPUT_DIR / "final_repo_binary_classifier_bundle.joblib")
    ...
    print("Saved final model artifacts to:", OUTPUT_DIR)
    print("- final_repo_binary_classifier_bundle.joblib")
    print("- final_model_trainset_predictions.csv")
